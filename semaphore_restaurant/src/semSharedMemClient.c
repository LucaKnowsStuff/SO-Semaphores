...
static bool waitFriends(int id)
{
    ...
    {
        for (int i = 0 ; i < (sh->fSt.tableClients - 1) ; i++) 
        {
    ...
    {
        if (semDown (semgid, sh->friendsArrived) == -1)                                                      /* exit critical region */
...
static void waitFood (int id)
{
    ...
    if (semDown (semgid, sh->foodArrived) == -1) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (foodArrived , CT)");
        exit (EXIT_FAILURE);
    }
